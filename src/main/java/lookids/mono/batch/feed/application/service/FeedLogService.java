...
public class FeedLogService implements FeedLogUseCase {
	...
	@Override
	public void feedCreateLog(FeedCreateEventDto feedCreateEventDto) {
		FeedLog feedLog = FeedLog.builder()
			...
			.processed(false)
			.build();
		feedRepositoryPort.createLog(feedDtoMapper.toFeedLogSaveDto(feedLog));
	}

	@Override
	public void feedDeleteLog(FeedDeleteEventDto feedDeleteEventDto) {
		FeedLog feedLog = FeedLog.builder()
			.feedCode(feedDeleteEventDto.getFeedCode())
			.uuid(feedDeleteEventDto.getUuid())
			.createdAt(feedDeleteEventDto.getCreatedAt())
